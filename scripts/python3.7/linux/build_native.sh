...
CURA_BUILD_ENV_PATH="${ROOT_DIR}/../cura-build-environment/install"
...
CURA_BUILD_OUTPUT_DIR="${CURA_BUILD_OUTPUT_DIR:-${ROOT_DIR}/../output}"
...
if [[ $1 == "clean" ]]; then
    rm -rf "${CURA_BUILD_OUTPUT_DIR}/build"
    rm -rf "${CURA_BUILD_OUTPUT_DIR}/appimages"
fi

mkdir -p "${CURA_BUILD_OUTPUT_DIR}/build"
mkdir -p "${CURA_BUILD_OUTPUT_DIR}/appimages"
...
cmake "${ROOT_DIR}" \
    ...
    -DAPPIMAGEKIT_APPIMAGETOOL_EXECUTABLE="${ROOT_DIR}/../appimagekit/appimagetool-x86_64.AppImage" \
